...
namespace Kunal\RatingFilter\Plugin\Review;

use Kunal\RatingFilter\Model\RatingSync;
    ...
    public function afterAggregate(ReviewResource $subject, $result, $object)
    {
        ...
        if ($productId) {
            try {
                ...
                $this->logger->error(
                    sprintf('Kunal_RatingFilter: could not sync product %d: %s', $productId, $e->getMessage()),
                    ['exception' => $e]
                );
            }
        }

...
class Validator
{
    ...
    public function validate(\Magento\Customer\Api\Data\AddressInterface $address) 
    {
        ...
        try {
            ...
            if (empty($result)) {
                $response['message'] = __(
                    'Your address could not be validated. Please enter a valid US address.'
                );
            } else {
                $response['valid'] = true;
            ...
            // If the account lacks an active subscription for address verification,
            // treat as pass-through (allow the address) to avoid blocking all orders.
            if (stripos($message, 'subscription') !== false
                || stripos($message, '1588026') !== false
                || $e->getCode() === 402
            ) {
                $this->logger->warning('SmartyStreets: ' . $message . ' - allowing address through.');
                $response['valid'] = true;
                $response['candidates'] = [];
            } else {
                // Received a real API error — block and show message.

...
class Service implements ServiceInterface
{
    ...
    public function get_prapi_params($type)
    {
        switch ($type)
        {
            ...
            case 'cart':
            case 'minicart':
                return json_encode($this->getApplePayParams($type));
            default: // Product
                $parts = explode(":", $type);
                ...
                if ($parts[0] == "product" && is_numeric($parts[1]))
                {
                    ...
                    return json_encode($this->getProductApplePayParams($parts[1], $attribute));
                }
                else
    ...
    public function place_multishipping_order($quoteId = null)
    {
        ...
        try
        {
            ...
            return json_encode(["redirect" => $redirectUrl]);
        }
        catch (SCANeededException $e)
        {
            return json_encode(["authenticate" => $e->getMessage()]);
        }
        catch (\Exception $e)
        {
            $this->paymentsHelper->logError($e->getMessage(), $e->getTraceAsString());
